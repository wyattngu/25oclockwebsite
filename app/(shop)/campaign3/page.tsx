...
import { getCampaign3Images } from "@/lib/utils/campaignImages";
import { CampaignSlideshow } from "@/components/home/CampaignSlideshow";
import { getT } from "@/lib/i18n/locale";

export async function generateMetadata(): Promise<Metadata> {
  const t = await getT();
  return { title: "Lookbook 0.3", description: t.lookbook.description3 };
}
...
export default async function Campaign3Page({ searchParams }: Props) {
  ...
  if (images.length === 0) {
    return (
      <div className="container-25 flex flex-col items-center gap-2 py-24 text-center">
        <h1 className="text-[20px] font-medium uppercase tracking-[0.06em]">Lookbook 0.3</h1>
